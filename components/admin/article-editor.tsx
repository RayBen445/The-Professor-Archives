...
import {
  ArrowLeft, Save, Eye, EyeOff, Star, StarOff, Feather,
  ChevronRight, Loader2, Check, ImageIcon, Type, AlignLeft, List,
  Bold, Italic, Quote, Heading1, Heading2, MapPin, Globe2, Clock, Tag,
} from "lucide-react";
...
export default function ArticleEditor({ articleId }: ArticleEditorProps) {
      ...
      fetch(`/api/articles/${articleId}`)
        ...
        .then((article: Article) => {
          setForm({
            ...
            read_time: article.read_time ? String(article.read_time) : "",
            tags: article.tags || "",
          });
          setAutoSlug(false);
          setLoading(false);
  ...
  const insertTag = (tag: string) => {
    ...
    let newText = "";
    switch (tag) {
      case "h2": newText = `<h2>${selectedText || "Heading"}</h2>`; break;
      case "h3": newText = `<h3>${selectedText || "Subheading"}</h3>`; break;
      case "bold": newText = `<strong>${selectedText || "bold text"}</strong>`; break;
      case "italic": newText = `<em>${selectedText || "italic text"}</em>`; break;
      case "quote": newText = `<blockquote>${selectedText || "Quote text"}</blockquote>`; break;
      case "p": newText = `<p>${selectedText || "Paragraph text"}</p>`; break;
      ...
      <header className="bg-ink text-cream sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-sm">{isEditing ? "Edit Story" : "New Story"}</span>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => updateField("published", !form.published)}
              className={`flex items-center gap-1.5 px-4 py-2 text-sm rounded-full border transition-all ${form.published ? "border-green-500/50 text-green-400 hover:bg-green-500/10" : "border-aged/30 text-aged hover:text-cream hover:border-cream/30"}`}>
              {form.published ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
              {form.published ? "Published" : "Draft"}
            </button>
            <button onClick={() => updateField("is_featured", !form.is_featured)}
              className={`flex items-center gap-1.5 px-4 py-2 text-sm rounded-full border transition-all ${form.is_featured ? "border-gold/50 text-gold hover:bg-gold/10" : "border-aged/30 text-aged hover:text-cream hover:border-cream/30"}`}>
              {form.is_featured ? <Star className="w-3.5 h-3.5 fill-current" /> : <StarOff className="w-3.5 h-3.5" />}
        ...
        <div className="space-y-6">
          ...
          <div>
            <input type="text" value={form.title} onChange={(e) => updateField("title", e.target.value)}
              placeholder="Story title..." className="w-full font-baby text-3xl md:text-4xl font-bold bg-transparent border-none outline-none text-ink placeholder:text-aged focus:ring-0 p-0" />
            <div className="flex items-center gap-2 mt-2">
              <span className="text-xs text-ink-muted">Slug:</span>
              <input type="text" value={form.slug} onChange={(e) => { setAutoSlug(false); updateField("slug", e.target.value); }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="text-xs text-ink-muted uppercase tracking-wider mb-1.5 block">Category</label>
            ...
            <div>
              <label className="text-xs text-ink-muted uppercase tracking-wider mb-1.5 block">Author</label>
            ...
            <div>
              ...
              <input type="text" value={form.image_url} onChange={(e) => updateField("image_url", e.target.value)} placeholder="https://..."
          ...
          {/* Meta row 2: new fields */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label className="text-xs text-ink-muted uppercase tracking-wider mb-1.5 flex items-center gap-1">
                <MapPin className="w-3 h-3" /> Country
              </label>
              <input type="text" value={form.country} onChange={(e) => updateField("country", e.target.value)} placeholder="e.g. Nigeria"
                className="w-full px-4 py-2.5 bg-cream border border-aged/30 rounded-xl text-ink text-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all placeholder:text-ink-muted" />
            </div>
            <div>
              <label className="text-xs text-ink-muted uppercase tracking-wider mb-1.5 flex items-center gap-1">
            ...
            <div>
              ...
              <input type="number" value={form.read_time} onChange={(e) => updateField("read_time", e.target.value)} placeholder="Auto"
                className="w-full px-4 py-2.5 bg-cream border border-aged/30 rounded-xl text-ink text-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all placeholder:text-ink-muted" />
            </div>
            <div>
              <label className="text-xs text-ink-muted uppercase tracking-wider mb-1.5 flex items-center gap-1">
                <Tag className="w-3 h-3" /> Published Date
              </label>
              <input type="date" value={form.published_date} onChange={(e) => updateField("published_date", e.target.value)}
                className="w-full px-4 py-2.5 bg-cream border border-aged/30 rounded-xl text-ink text-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all" />
            </div>
          </div>
          ...
          <div>
            ...
            <input type="text" value={form.tags} onChange={(e) => updateField("tags", e.target.value)} placeholder="e.g. colonialism, Africa, resistance"
              className="w-full px-4 py-2.5 bg-cream border border-aged/30 rounded-xl text-ink text-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all placeholder:text-ink-muted" />
          </div>
          ...
          <div>
            ...
            <textarea value={form.excerpt} onChange={(e) => updateField("excerpt", e.target.value)}
              placeholder="A brief summary of the story..." rows={3}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1 mb-2 p-2 bg-cream border border-aged/30 border-b-0 rounded-t-xl">
              {[
                ...
                <button key={item.tag} onClick={() => insertTag(item.tag)}
                  className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-ink-light hover:text-accent hover:bg-accent/5 rounded-lg transition-all" title={item.label}>
                  <item.icon className="w-3.5 h-3.5" />{item.label}
                </button>
              ))}
            </div>
            <textarea id="content-editor" value={form.content} onChange={(e) => updateField("content", e.target.value)}
              placeholder="<p>Write your story here...</p>" rows={20}
